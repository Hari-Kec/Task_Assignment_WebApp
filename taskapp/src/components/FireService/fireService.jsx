...
const FireService = () => {
  const userId = '111'; // Replace with actual user ID
  ...
  const moveTask = (task, source, target, newStatus) => {
    ...
    axios.put(`https://3.26.234.195:5000/fireservice/${task._id}`, { ...task, status: newStatus })
  ...
  const handleFormSubmit = (e) => {
    ...
    axios.post('https://3.26.234.195:5000/fireservice', task)
      .then((response) => {
  ...
  const deleteTask = (taskId, status) => {
    axios.delete(`https://3.26.234.195:5000/fireserviceTasks/${taskId}`)
      .then(() => {
  ...
  const handleSafetyMeasuresClick = () => {
    navigate('/fireservice-safety'); // Navigate to the safety measures page
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="bg-gray-100 text-black shadow-md w-full fixed top-0 left-0 z-10">
        <div className="max-w-full px-4 py-4 flex justify-between items-center">
          <h1 className="text-2xl font-semibold">Fire Service</h1>
