...
const FireService = () => {
  ...
  const moveTask = (task, source, target, newStatus) => {
    ...
    axios.put(`http://localhost:5000/fireservice/${task._id}`, { ...task, status: newStatus })
  ...
  const handleFormSubmit = (e) => {
    ...
    axios.post('http://localhost:5000/fireservice', task)
      .then((response) => {
  ...
  const deleteTask = (taskId, status) => {
    axios.delete(`http://localhost:5000/fireserviceTasks/${taskId}`)
      .then(() => {
